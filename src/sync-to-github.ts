...
const EXCLUDE = [/^\.local\//, /^\.cache\//, /node_modules\//, /\/dist\//, /^dist\//, /\.replit-artifact\//, /\.tsbuildinfo$/];
...
async function main(): Promise<void> {
  ...
  const listed = execSync("git ls-files --cached --others --exclude-standard", { maxBuffer: 1 << 26 })
    .toString()
    .split("\n")
    .filter(Boolean);
