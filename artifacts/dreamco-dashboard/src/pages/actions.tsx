...
export default function ActionsPage() {
  ...
  const TILES: Tile[] = [
    ...
    { id: "fleet", title: "Bot Fleet", icon: <BotIcon className="h-5 w-5" />, status: "live", blurb: "All 171 bots, Run buttons live.", href: "/bots", metric: summary.data ? `${summary.data.activeBots} active / ${summary.data.totalBots}` : "—" },
    { id: "deals", title: "Deal Analyzer", icon: <BarChart3 className="h-5 w-5" />, status: "planned", blurb: "Real estate + business deal scoring.", note: "Wire to real_estate_bot — ~4hr" },
    { id: "formulas", title: "Formula Vault", icon: <BookOpen className="h-5 w-5" />, status: "live", blurb: "Library mastery + revolutionary uses + math basis.", href: "/vibe", metric: vibeStats.data ? `${vibeStats.data.librariesLearned} libs / ${vibeStats.data.ideasGenerated} ideas` : "—" },
    { id: "learning", title: "Learning Matrix", icon: <GraduationCap className="h-5 w-5" />, status: "live", blurb: "Lesson-mode kid games (parent input).", href: "/vibe", metric: "mode=lesson on Vibe Engine" },
    { id: "ai-leaders", title: "AI Leaders", icon: <Brain className="h-5 w-5" />, status: "planned", blurb: "Top AI models by benchmark.", note: "Live LMArena scrape — ~3hr" },
    ...
    { id: "ecosystem", title: "AI Ecosystem", icon: <Globe className="h-5 w-5" />, status: "live", blurb: "Global library reach (40+ langs, CN/JP/RU/KR/IN).", href: "/vibe", metric: vibeStats.data ? `${vibeStats.data.ecosystems.length} ecosystems` : "—" },
    { id: "orchestration", title: "Orchestration", icon: <Workflow className="h-5 w-5" />, status: "live", blurb: "GitHub Actions workflow runs across all repos.", metric: actions.data ? `${actions.data.totals.totalRuns} runs · ${actions.data.totals.success} ok` : "—" },
    { id: "marketplace", title: "Marketplace", icon: <ShoppingBag className="h-5 w-5" />, status: "planned", blurb: "Paid bot listings + buyer flow.", note: "Stripe Connect + listing schema — ~8hr" },
    { id: "crypto", title: "Crypto", icon: <Bitcoin className="h-5 w-5" />, status: "needs_key", blurb: "Wallet balances + price feeds.", note: "Needs COINBASE_API_KEY or ALCHEMY_API_KEY" },
    ...
    { id: "biz", title: "Biz Launch", icon: <Rocket className="h-5 w-5" />, status: "planned", blurb: "Spin up a new bot-driven business in N steps.", note: "Wizard + bot bundling — ~6hr" },
    ...
    { id: "loans", title: "Loans & Deals", icon: <Banknote className="h-5 w-5" />, status: "planned", blurb: "Aggregate SBA / business loan feed.", note: "External API + scraper — ~6hr" },
    ...
    { id: "time", title: "Time Capsule", icon: <Clock className="h-5 w-5" />, status: "planned", blurb: "Daily snapshots of metrics for investor decks.", note: "Cron + JSON snapshot table — ~3hr" },
    { id: "costs", title: "Cost Tracking", icon: <Wallet className="h-5 w-5" />, status: "live", blurb: "AI token usage per build (sum of tokens_used).", href: "/vibe", metric: vibeStats.data ? `${vibeStats.data.buildsCompleted} builds tracked` : "—" },
    { id: "autonomy", title: "Autonomy", icon: <Shield className="h-5 w-5" />, status: "planned", blurb: "Toggle Buddy's autonomous action level (read / suggest / act).", note: "Settings row + middleware gate — ~2hr" },
  ];
